...
void venderCripto(char *usuario, struct moedas *informacoes) {
  ...
  while (continuar) {
    ...
    printf("Criptomoedas disponíveis para venda:\n");
    ...
    while (!valido) {
      ...
      printf("Digite a quantidade que irá vender (ou 0 para cancelar): ");
      fgets(input, sizeof(input), stdin);
      ...
      if (quantidade > saldoDisponivel) {
        printf("Quantidade insuficiente :(, você possui apenas %.8f %s\n",
               saldoDisponivel, criptoEscolhida);
        continue;
      }
    ...
    // Aplica taxa
    float taxa = (strcmp(criptoEscolhida, "bitcoin") == 0)    ? 0.03
                 : (strcmp(criptoEscolhida, "ethereum") == 0) ? 0.02
                                                              : 0.01;
    ...
    snprintf(detalhes, sizeof(detalhes),
             "VENDA_%s (QTD: %.8f, COTAÇÃO: R$ %.2f)", criptoEscolhida,
             quantidade, cotacaoEscolhida);
    registrarTransacao(usuario, "VENDA_CRIPTO", valorLiquido, detalhes);
    ...
    printf("\nDeseja vender outra criptomoeda? (1-Sim / 0-Não): ");
    fgets(input, sizeof(input), stdin);
